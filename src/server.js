...
const runBackupSync = async () => {
  try {
    logger.info('Running database backup sync in-process...');
    ...
    await fs.promises.writeFile(outPath, JSON.stringify(backupData, null, 2), 'utf8');
    logger.info('Successfully wrote sync data in-process to: ' + outPath);
  } catch (error) {
    logger.error('Error during in-process sync backup:', error);
  }
};

const triggerBackupSync = () => {
  runBackupSync().catch(err => {
    logger.error('Failed to trigger in-process backup sync:', err);
  });
};
